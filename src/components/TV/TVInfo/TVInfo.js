...
class TVInfo extends Component {
  getTVShow = () => {
    ...
    const tvId = this.props.params.id;
  ...
  render() {
    ...
    const { tv } = this.props;
    let tvTrailer = tv.videos.results.filter(
      (result) => result.type === "Trailer"
    );
    if (tvTrailer.length === 0) {
      tvTrailer = tv.videos.results.filter(
    ...
    const tvData = {
      ...
      other: [
        ...
        {
          name: "Networks",
          value: tv.networks.map((network) => network.name).join(", "),
        },
        {
    ...
    return (
      <MediaInfo data={tvData} onCardClickHandler={this.onCardClickHandler} />
    );
  }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(TVInfo));
